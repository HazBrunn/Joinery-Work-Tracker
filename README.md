...
3. **Authentication** → Get started → enable the **Anonymous** sign-in provider. (The app signs in
   anonymously so the security rules above can require an authenticated request — no login screen.)
...
- `firebase` (Firestore + Storage + anonymous Auth) for the optional cloud backend
...
1. Get the code onto **GitHub** (new repo → push), then on [vercel.com](https://vercel.com) →
   **Add New → Project** → import the repo. Vercel auto-detects Vite.
...
3. **Deploy.** Then in the Firebase console → **Authentication → Settings → Authorized domains**,
   add your new `*.vercel.app` domain so sign-in keeps working everywhere.
